...
func TestCreateUserWithAccountRoutesPhysicalTable(t *testing.T) {
	...
	userID, shardNo := userIDInShardRangeForTest(t, 300, 399)
	tableName, err := UserPhysicalTableName(shardNo, 10)
	if err != nil {
		t.Fatalf("UserPhysicalTableName() error = %v", err)
	}
	if tableName != "user_300" {
		t.Fatalf("route table = %s, want user_300 for shard=%d", tableName, shardNo)
	}
	...
	if err = CreateUserWithAccount(db, user, 10); err != nil {
	...
	if account == nil || account.RouteShardCount != 10 || account.ShardNo != shardNo {
		t.Fatalf("account = %+v, want shard=%d route=10", account, shardNo)
	}
}
