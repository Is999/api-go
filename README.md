...
API 迁移使用独立版本号段，避免和同库的 admin 迁移版本冲突。
...
- 安全链路：`security.secret_key` 配置秘钥后启用 `X-App-Id`、`X-Signature`、`X-Crypto`、`X-Cipher`、`X-Key-Version` 等请求头；未配置秘钥时仍允许普通 JSON 请求。
...
前台用户表使用 `user`，不再额外添加 `api_` 前缀；后台管理员表由 `admin` 项目维护，通过业务表名区分。运行期系统配置表固定使用 `sys_config`；后续新增表默认使用清晰业务名，只有特殊业务边界需要时才使用业务前缀。
